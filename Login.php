...
<div class="container features">
	...
	if (isset($_POST["button1"])){
		...
		$errors+=1;
			...
			if($db_found){
				//on cherche dans acheteur la personne qui a cet Email et password
				if ($Email != "" ) {
						$sql="SELECT * FROM acheteur  
						WHERE Email LIKE '$Email' ";
						if ($Password != ""){
							$sql="SELECT * FROM acheteur  
						WHERE Email LIKE '$Email' AND Mot_de_passe LIKE '$Password' ";
				...
				if (mysqli_num_rows($result) == 0) {
					echo '<h5>Un de vos identifiants ou les deux sont incorrects. Veuillez vérifier!</h5>
					<h5>Si vous n\'avez pas de compte EbayECE créez-en un en cliquant <a href="signup.html">ici</a>!</h5>';	
				}			
				else{
					echo '<h5>Votre connexion est réussie. Vous pouvez maintenant profitez pleinement pour votre achat! </h5>';
				}
			}
		}
	}
